...
public class SocioAfectivaDaoImpl implements IDAO{
    ...
    public String modificar(Object obj) throws SQLException {
        SocioAfectiva objsociofectiva =(SocioAfectiva) obj;
        try {
            psmt = con.conectar().prepareStatement("UPDATE SocioAfectiva SET nombre=? AND estado=? WHERE idSocioAfectiva=?" );
            psmt.setInt(1,objsociofectiva.getIdSocioAfectiva());
            psmt.setString(2, objsociofectiva.getNombre());
            psmt.setString(3, objsociofectiva.getEstado());
            psmt.executeUpdate();
